...
```{r}
anov <- lm(Churn.Score~Senior.Citizen+Payment.Method, data=telco)
summary(anov)
```
```{r}
m1 <- lm(Churn.Score~Paperless.Billing+Senior.Citizen+Payment.Method+Partner+Dependents+Tech.Support+Internet.Service+Online.Security+Contract, data=telco)
summary(m1)
```
...
```{r}
m2 <- lm(Churn.Score~Tenure.Months+Monthly.Charges+Total.Charges, data = telco)
summary(m2)
```
