...
payment method--electron check
contract---monthly(A)
InternetServices --Fiber Optic(A)
OnlineSecurity ---no
OnlineBackup--no
TechSupport ---no
Partners---no
...
```{r ,results='markup'}
x <- table(Adata2$admit,Adata2$rank)
x
#Both the variables are qualitative variables
...
#p-value is less than 0.05 so sonclusion is not independent
```
