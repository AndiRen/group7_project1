...
These are the variable which are in this data set - tenure months, monthly charges, avg monthly  long distance , total extra , avg monthly  download .

Here we have created two table for churn and non-churn containing MIN, Q1, MEDIAN, MEAN, Q3, and MAX.

One important point to be noted from these tables is for the monthly charges, avg monthly long distance charges avg monthly gb downloaded the mean and median are almost equal or close in both churn and non-churn group. 

From this we can infer that the graphs for these variables are symmetric.

Also, from these tables, we can observe that the mean and median for tenure and average monthly charges are different for churn and non-churn group. From this we can infer that the factors tenure and average monthly charges has a weight over the decision of customers in deciding whether to churn or not.

The point here to be noted here that, these are some few bits of information including mean, median, max, min, and we cannot solely infer conclusion on the basis of these few information alone, but as an initial step we can take these inferences. The subsequent statistical analysis and tests are explained in the upcoming sections.
...
Now that we've looked at the descriptive statistics of the data, we will explore the data graphically. In all instances we divided the customers into churn and non-churn groups. For categorical variables, we look at the difference in frequency distribution for the levels of the given category between churn and non-churn. For continuous variables, we look at graphs of the distribution of the data through histograms and boxplots, as well as checking for normality in the distribution with qq-plots. We note whether the shape of the graphs would indicate that the frequency of a variable's levels differ between churn and non-churn for the categorical variables and whether the distribution of the data looks different between churn and non-churn for the continuous variables.
...
The x-axis of this graph contains the churn and non-churn groups. The y-axis displays the frequency distribution of senior citizens in each group.
...
The x-axis contains the levels of the variable. The y-axis contains the frequency of churn or non-churn for that level.
...
The x-axis of this graph contains the churn and non-churn groups. The y-axis displays the frequency distribution of the different contract types in each group.
...
The x-axis of this graph contains the churn and non-churn groups. The y-axis displays the frequency distribution of the satisfaction scores in each group.
...
For example, from the earlier analysis on Techsupport, online backup and online security. Customers who do not have service like online backup, online security and tech support have higher churn rate as compared to other options. This make a intuitive sense that customers are not willing to stay with a telecom platform that have poor performance on customer support system. Therefore, we assume these three variables have some capability to predict some part of target value. 


Another example being, the customers who have a dependency has a very low churn rate than customers who have no dependency.
The low p value from the chi sq test also confirm variable dependents has an weightage on deciding people to stay or leave
Apart from the statistical analysis, this can be explained as :

1. If a customer who has dependency need to leave , everyone in the dependency should have to leave all together ,which is more complicated than customer who has no dependents , and who can easily leave the service.

2. Also the service provider may be providing a better plan for customer who has dependents, than their other service competitors.

A third example would be Satisfaction Score. The chi squared test of independence for Satisfaction Score revealed that Satisfaction Score was not independent of the decision to churn. Given the near total lack of overlap in ratings between the churn and non-churn groups, it is likely that satisfaction Score would return a high r^2 value in a model, meaning that differences in Satisfaction Score could account for much of the differences in the decision to churn. However, when one thinks about the context, of course unhappy customers left and customers who stayed tended to be more satisfied. Satisfactions Score is almost a corollary with churn and non-churn. What we really want to know is what is associated with a customer being so unsatisfied that they left. Thus, we will likely not use Satisfaction Score in building our model, but rather to inform us on how to understand what the model tells us.  
