...
```{r}
confusionMatrix(table(test_pred_grid, testing$filtered_joined_churn_df.Churn.Label))
```
...
### Linear SVM
svm_linear1 <- train(filtered_joined_churn_df.Churn.Label ~., data = train_data, method = "svmLinear", trControl=trainControl(method = "repeatedcv", number = 10, repeats = 3), preProcess = c("center", "scale"), tuneLength = 10)
svm_linear1
test_pred1<-predict(svm_linear1,test_data)
confusionMatrix(table(test_pred1,test_label))
...
##### non linear SVM
####Radial kernel
svm_Radial<-train(filtered_joined_churn_df.Churn.Label ~., data = train_data, method = "svmRadial", trControl=trainControl(method = "repeatedcv", number = 10, repeats = 3), preProcess = c("center", "scale"), tuneLength = 10)
