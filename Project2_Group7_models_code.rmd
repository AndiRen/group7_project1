...
pred_cutoff <- ifelse(predictions_final_model > 0.28, 1, 0)
...
xkabledply(confusion_matrix_cutoff_df, title="Confusion Matrix for Final Model With a Cut Off Value of 0.28")
...
* The overall avg Monthly Charge of ~$65, has a 3.5-fold growth effect on the odds-ratio of a customer churning
...
The overall accuracy is `r round(cm$overall["Accuracy"]*100, digits=2)`%
...
set.seed(1)
treefit <- rpart(Churn.Label ~ Contract+Dependents+Internet.Service+prob+Satisfaction.Score, data=filtered_joined_churn_df, method="class", control = rpart.control(minsplit =1,minbucket=1, cp=0 ))
# treefit <- rpart(Kyphosis ~ Age + Number + Start, data=filtered_joined_churn_df, method="class", control = {rpart.control list} )
# rpart.control(maxdepth = 30, minsplit = 20, minbucket = round(minsplit/3), cp = 0.01, maxcompete = 4, maxsurrogate = 5, usesurrogate = 2, xval = 10, surrogatestyle = 0, ...)
...
# plot tree 
plot(treefit, uniform=TRUE, main="Classification Tree 2 for Churn")
...
loadPkg("caret") 
cm = confusionMatrix( predict(treefit, type = "class"), reference = as.factor(filtered_joined_churn_df$Churn.Label) )
print('Overall: ')
cm$overall
print('Class: ')
cm$byClass
...
# treefit <- rpart(Kyphosis ~ Age + Number + Start, data=kyphosis, method="class", control = {rpart.control list} )
# rpart.control(maxdepth = 30, minsplit = 20, minbucket = round(minsplit/3), cp = 0.01, maxcompete = 4, maxsurrogate = 5, usesurrogate = 2, xval = 10, surrogatestyle = 0, ...)
...
for (deep in 2:6) {
  kfit <- rpart(Churn.Label ~ Contract+Dependents+Internet.Service+prob+Satisfaction.Score, data=filtered_joined_churn_df, method="class", control = list(maxdepth = deep) )
  # 
  cm = confusionMatrix( predict(kfit, type = "class"), reference = as.factor(filtered_joined_churn_df$Churn.Label)) # from caret library
  # 
  cmaccu = cm$overall['Accuracy']
...
xkabledply(confusionMatrixResultDf, title="Churn Classification Trees2 summary with varying MaxDepth")
