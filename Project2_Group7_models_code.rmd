...
```{r results='markup'}
library(pROC)
ROC_rf1 <- roc(test_data$filtered_joined_churn_df.Churn.Label, test_pred2[,2])
ROC_rf_auc1 <- auc(ROC_rf1)

plot(ROC_rf1, col = "green", main = "ROC Curve for the prune tree")


paste("Accuracy : ", mean(test_data$filtered_joined_churn_df.Churn.Label == round(test_pred[,2], digits = 0)))

paste("Area under curve for prune tree model is: ", ROC_rf_auc1)
