...
#Monthly.Charges + Contract + Dependents

#Build model
adj_logit_model <- glm(Churn.Label ~ Dependents + Phone.Service + Contract + Monthly.Charges + Avg.Monthly.GB.Download, data=filtered_joined_churn_df, family="binomial")
summary(adj_logit_model)
xkabledply(adj_logit_model, title="Churn.Label ~ All Factors")
...
```{r results='markup'}
#exponentiate both sides of the equation
expcoeff <- exp(coef(adj_logit_model))
xkabledply(as.table(expcoeff), title='Growth and Decay Factors After Exponentiation') 
```

**Confusion Matrix: adjusted features**

```{r}
#Turn from kable into data frame (was having trouble accessing elements as a kable)
confusion_matrix_adj_df <- data.frame(confusion_matrix(adj_logit_model))
#confusion_matrix_adj_df

accuracy <- (confusion_matrix_adj_df[1,1] + confusion_matrix_adj_df[2,2])/confusion_matrix_adj_df[3,3]
precision <- confusion_matrix_adj_df[2,2]/(confusion_matrix_adj_df[2,2] + confusion_matrix_adj_df[1,2])
recall_rate <- confusion_matrix_adj_df[2,2]/(confusion_matrix_adj_df[2,2] + confusion_matrix_adj_df[2,1])
specificity <- confusion_matrix_adj_df[1,1]/(confusion_matrix_adj_df[1,1] + confusion_matrix_adj_df[1,2])
f1_score <- 2*(precision)*(recall_rate)/(precision + recall_rate)
...
```{r}
#Get actual values of churn/non-churn
predictions_adj_model <- adj_logit_model$fitted.values
#Get predictions based off of selected cut off value
pred_cutoff <- ifelse(predictions_adj_model > 0.26, 1, 0)
