...
```{r}
intrain <- createDataPartition(y = my_data$filtered_joined_churn_df.Churn.Label, p= 0.7, list = FALSE)
...
```{r}
svm_Linear <- train(filtered_joined_churn_df.Churn.Label ~., data = my_data, method = "svmLinear", trControl=trctrl, preProcess = c("center", "scale"), tuneLength = 10)
svm_Linear
```
...
```{r}
confusionMatrix(table(test_pred, testing$filtered_joined_churn_df.Churn.Label))
```

```{r}
grid <- expand.grid(C = c(0,0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2,5))
svm_Linear_Grid <- train(filtered_joined_churn_df.Churn.Label ~., data = my_data, method = "svmLinear",
trControl=trctrl,
