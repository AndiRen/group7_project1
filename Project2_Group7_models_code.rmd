...
```{r}
xkabledplyhead(filtered_joined_churn_df)
```

```{r}
# Check the composition of labels in the data set. 
table(filtered_joined_churn_df$`Churn.Label`)
table(filtered_joined_churn_df$`Churn.Label`)[2] / sum(table(filtered_joined_churn_df$`Churn.Label`))
```

```{r}
set.seed(1)
train_rows <- sample(1:nrow(filtered_joined_churn_df), round(0.8 * nrow(filtered_joined_churn_df), 0), replace = FALSE)
```

```{r}
length(train_rows) / nrow(filtered_joined_churn_df)
```

```{r}
train = filtered_joined_churn_df[train_rows, ]  
test = filtered_joined_churn_df[-train_rows, ]
...
```{r}
loadPkg("class")
data_3NN = knn(train = train[, c("Senior.Citizen", "Internet.Service", "Phone.Service", "Tenure.Months", "Monthly.Charges")], test = test[, c("Senior.Citizen", "Internet.Service", "Phone.Service", "Tenure.Months", "Monthly.Charges")], cl = train[, "Churn.Label"], k = 3)
```
...
```{r}
vars <- c("Senior.Citizen","Churn.Label", "Partner", "Dependents","Tenure.Months","Phone.Service","Online.Security","Online.Backup","Tech.Support","Paperless.Billing","Unlimited.Data","Avg.Monthly.Long.Distance.Charges","Avg.Monthly.GB.Download","Monthly.Charges","Tenure.Months")
```
...
#Partition of data
```{r}
train_sample<-sample(1:nrow(filtered_joined_churn_df),nrow(filtered_joined_churn_df)*0.7)
train_data<-filtered_joined_churn_df[train_sample, ]
test_data<-filtered_joined_churn_df[-train_sample, ]
train_label<-filtered_joined_churn_df[train_sample, 14]
test_label<-filtered_joined_churn_df[-train_sample, 14]
```


#Linear SVM
```{r}
svm_linear<-train(("Churn.Label") ~., data = train_data, method = "svmLinear", trControl=trainControl(method = "repeatedcv", number = 10), preProcess = c("center", "scale"), tuneLength = 10)
```
...
#no
```{r}
intrain <- createDataPartition(y = vars[[Churn.Label]], p= 0.7, list = FALSE)
