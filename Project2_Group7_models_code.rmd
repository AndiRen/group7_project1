...
$\frac{p}{q} = `r format(expcoeff[1], digits=6)` * (`r format(expcoeff[2], digits=6)`)^{senior} * (`r format(expcoeff[3], digits=6)`)^{partner} * (`r format(expcoeff[4], digits=6)`)^{dependent} * (`r format(expcoeff[5], digits=6)`)^{tenure} * (`r format(expcoeff[6], digits=6)`)^{phone}$  
...
# Features
x <- filtered_joined_churn_df %>%
  select(-Churn.Label) %>%
  as.data.frame()

# Target variable
y <- filtered_joined_churn_df$Churn.Label
...
typeof(filtered_joined_churn_df)
...
#x_train = as.vector(unlist(c(x_train)))

#y_train = as.vector(unlist(c(y_train)))
...
# Run RFE
#result_rfe1 <- rfe(x = x_train, y = y_train, sizes = c(1:8),rfeControl = control)
