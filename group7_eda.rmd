...
**From the graph below, we see that around 26% of customer left the platform within the last month.**
...
Focusing in on the Churn Customers vs. Non-Churn Customers summary tables, for many categorical variables [there appears to be a difference in the frequency of churn customers who beloing to a given variable's level]{.underline}. For example, there are 1493 Churn Customers who are senior citizens and 476 who are not (~24%). For Non-Churn Customers, there are 4508 senior citizens and 666 non-seniors (~13%). While there are fewer non-senior's overall in the Churn Customer group, the percentage of seniors is higher compared to the Non-Churn group. The difference in frequency of these variables' levels will need to be analyzed.
...
Since `Internet Services` has more than two levels, a chi squared, [test of independence]{.underline} was used. A contingency table was created comparing Churn/Non-Churn against the Satisfaction Ratings. Then, this table was passed to the chisq.test() function to get the result of the test. Thus the hypothesis is as follows:  
